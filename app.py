...
live_refresh = st.sidebar.toggle(
    ...
    key="sb_refresh_v2",
    help=(
        "Off by default because a full-page rerun can temporarily blank the dashboard while market APIs respond. "
        "The header ticker still refreshes independently every 5 seconds."
    ),
)
...
def _fetch_upstox_history_impl(instrument_key, token, days=400):
    ...
    if UPSTOX_SDK_AVAILABLE:
        try:
            ...
            ranged_method = getattr(api, "get_historical_candle_data1", None)
            legacy_method = getattr(api, "get_historical_candle_data", None)
            # The SDK call used to be unbounded. If its socket stalled, the
            # worker never reached the timeout-controlled REST fallback and a
            # scan appeared frozen on whichever ticker had completed last.
            # Upstox's generated client forwards _request_timeout to urllib3.
            sdk_timeout = (3, 7)  # connect, read seconds
            if callable(ranged_method):
                resp = ranged_method(
                    instrument_key, "day", end_date.isoformat(),
                    start_date.isoformat(), "2.0",
                    _request_timeout=sdk_timeout,
                )
            elif callable(legacy_method):
                resp = legacy_method(
                    instrument_key, "day", end_date.isoformat(), "2.0",
                    _request_timeout=sdk_timeout,
                )
            else:
                resp = None
    ...
    try:
        ...
        headers = {"Accept": "application/json", "Authorization": f"Bearer {token}"}
        # Historical scans can fan out across many symbols. One retry plus a
        # bounded connect/read timeout prevents a temporary Upstox slowdown
        # from multiplying into minutes of silent waiting per worker. Cached
        # history remains the existing fallback in get_cached_history().
        with get_robust_session(total_retries=1) as session:
            res = session.get(url, headers=headers, timeout=(3, 7))
            if res.status_code != 200:
    ...
    try:
        with get_robust_session() as session:
            for i in range(0, len(keys_list), CHUNK_SIZE):
                ...
                headers = {'accept': 'application/json', 'Authorization': f'Bearer {token}'}
                try:
                    res = session.get(url, headers=headers, params={'instrument_key': keys_str}, timeout=8)
                    if res.status_code == 200:
...
# Render this before any main-area network calls so a slow/cold API response
# can never leave the entire first page looking empty.
# ==========================================
nav_options = [
...
startup_status.info("Loading live market data… The page will remain visible while connections complete.")
...
# ==========================================
    ...
    if scan_mode.startswith("Full") and not _scan_result_available:
        st.info(
        ...
        if _universe > 0 and _quote_coverage_pct < 20:
            ...
            st.error(
                f"⚠️ **Full NSE analysis incomplete due to data/API failures.**\n\n"
            ...
            st.error(
                f"⚠️ **Full NSE scan could not produce a shortlist.**\n\n"
            ...
            st.warning(
                f"⚠️ **Full NSE analysis timed out.** {_timed_out}/{_submitted} candidates did not complete within "
            ...
            st.error(
                f"⚠️ **Full NSE analysis incomplete due to unexpected errors.** {_worker_exc}/{_completed} completed "
            ...
            st.warning(
                f"✅ **Full NSE scan completed successfully. No candidates passed the technical criteria.**\n\n"
