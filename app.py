...
with st.sidebar.expander("⚡ True-Live Ticker (advanced, optional)"):
    st.caption(
        "Connects the price ticker directly to a relay server via browser WebSocket — genuine "
        "tick-by-tick updates with no page rerun, closer to a real brokerage app. Requires deploying "
        "relay_server.py separately (see the file Claude provided) and entering its details below."
    )
    relay_url = st.text_input("Relay URL (wss://...)", key="sb_relay_url", placeholder="wss://your-relay.onrender.com")
    relay_secret = st.text_input("Relay Shared Secret", key="sb_relay_secret", type="password")

use_true_live = bool(relay_url and relay_secret)

if use_true_live:
    _render_true_live_ticker(relay_url, relay_secret)
elif HAS_FRAGMENT:
