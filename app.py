...
APP_BUILD = "v15.0-MUTUAL-FUND-RANKING"
...
# The owner requested a deployment without application-level login. Personal
# records still use one stable internal owner id, but this is not multi-user
# isolation. Keep the Streamlit deployment private at the hosting layer.
CURRENT_USER_ID = hashlib.sha256(b"single-user-deployment").hexdigest()
CURRENT_USER_DISPLAY = "Single-user mode"
...
def get_sector_bucket(ticker):
    """Do not treat unrelated unknown companies as one correlated sector."""
    ticker = str(ticker or "").upper()
    return NSE_SECTOR_MAP.get(ticker, f"Unclassified:{ticker}")
...
if primary_section == "Settings":
    ...
    if hasattr(st, "segmented_control"):
        selected_tab = st.segmented_control(
            "View", _subpages, default=_subpages[0], key=f"subpage_{primary_section}",
        )
    else:
...
if primary_section == "Settings":
    ...
    with st.sidebar.expander("My Positions"):
        st.caption("Private to your authenticated account. Used only for sector-exposure warnings.")
...
UPSTOX_RATE_LIMITER = SlidingWindowRateLimiter()
...
UPSTOX_CIRCUIT_BREAKER = ProviderCircuitBreaker()
...
AI_REQUEST_QUOTA = PerUserQuota(limit=10, window_seconds=60.0)
...
def fetch_upstox_intraday_series(instrument_key, token, unit, interval, days_back=10):
    ...
    try:
        ...
        headers = {"Accept": "application/json", "Authorization": f"Bearer {token}"}
        with get_robust_session(total_retries=0) as session:
            res = upstox_request("GET", url, session=session, headers=headers, timeout=(3, 8))
            if res.status_code != 200:
...
class MarketDataBuffer:
    ...
    def ensure(self, keys):
        ...
        if not keys or not self.token or not UPSTOX_SDK_AVAILABLE:
            return
        with self.lock:
            # Upstox V3 currently permits up to 5,000 LTPC instruments when
            # LTPC is the only subscribed category. Subscribe only genuinely
            # new keys so Streamlit reruns do not send duplicate requests.
            new_keys = [k for k in keys if k not in self.subscribed]
            available_slots = max(5000 - len(self.subscribed), 0)
            if len(new_keys) > available_slots:
                self.last_error = (
                    f"LTPC subscription capped at 5,000 instruments; "
                    f"{len(new_keys) - available_slots} key(s) require REST quotes"
                )
                new_keys = new_keys[:available_slots]
            self.subscribed.update(new_keys)
        ...
        while i + horizon_days + 1 < n:
            ...
            for _, bar in future_slice.iterrows():
                ...
                if hit_target:
                    ...
                    break
            if outcome is None:
                net_return_pct = (float(future_slice['Close'].iloc[-1]) / entry - 1.0) * 100.0 - float(transaction_cost_pct)
                outcome = 1 if net_return_pct > 0 else 0
...
def _fetch_mf_scheme_list_cached():
    ...
    try:
        with get_robust_session(total_retries=1) as session:
            response = session.get(MF_TIGZIG_SNAPSHOT_URL, headers=MF_HTTP_HEADERS, timeout=(5, 20))
    ...
    try:
        with get_robust_session(total_retries=1) as session:
            response = session.get(MF_AMFI_SCHEME_URL, headers=MF_HTTP_HEADERS, timeout=(5, 15))
            if response.status_code == 200:
    ...
    try:
        with get_robust_session(total_retries=1) as session:
            response = session.get(MF_MFAPI_BASE_URL, headers=MF_HTTP_HEADERS, timeout=(5, 12))
            if response.status_code == 200:
...
def _fetch_mf_nav_history_cached(scheme_code):
    """Fetch five-plus years of NAV history; transient failures are not cached."""
    code = str(scheme_code).strip()
    since = (datetime.date.today() - datetime.timedelta(days=(366 * 5) + 31)).isoformat()
    provider_errors = []
...
def _fetch_mf_nav_histories_bulk_cached(scheme_codes):
    """Fetch up to five years for many schemes in rate-efficient groups of 50."""
...
def compute_mf_returns(nav_json):
    ...
    try:
        ...
        freshness_days = max(0, (pd.Timestamp(datetime.date.today()) - latest_date.normalize()).days)
    ...
    for i, idx_name in enumerate(selected_indices):
        with cols[i]:
            ...
            if key and live_quotes and key in live_quotes:
                ...
                st.caption(f"⚪ {idx_name}: waiting for live feed")
...
if primary_section != "Settings" and HAS_FRAGMENT:
...
if primary_section != "Settings":
    ...
    vix_value = 15.0
    volatility_regime = "Unavailable in Settings"
    nifty_hist_df = pd.DataFrame()
...
if selected_tab == "Settings":
    ...
    sec1.metric("Access mode", "Single user / no login")
    ...
    with st.expander("Prediction model production standard"):
        st.markdown(
            "1. Define 5/15/30-session targets before feature engineering: excess return, return quantiles, and first target/stop hit.\n"
            "2. Build a point-in-time dataset with historical NSE membership, delisted stocks, corporate actions, sector data, "
            "adjusted OHLCV, derivatives positioning and event flags.\n"
            "3. Use purged walk-forward validation with an embargo, realistic fees, spread, slippage and liquidity constraints.\n"
            "4. Compare an interpretable logistic baseline with a gradient-boosting challenger; calibrate probabilities.\n"
            "5. Report Brier score, log loss, calibration, precision@K, rank IC, turnover, drawdown and uncertainty intervals.\n"
            "6. Monitor data freshness, feature drift and calibration decay. Gemini may explain verified outputs but must not produce prices."
        )
...
# ==========================================
    ...
    if "scan_session_id" not in st.session_state:
        st.session_state["scan_session_id"] = hashlib.sha256(
            f"{CURRENT_USER_ID}:{time.time_ns()}:{threading.get_ident()}".encode("utf-8")
        ).hexdigest()[:20]
    _scan_coordinator = get_scan_coordinator()
    _scan_owner = st.session_state["scan_session_id"]
    if run_scan_now and not _scan_coordinator.try_start(_scan_owner, lease_seconds=150):
        st.warning("Another market scan is already running. Please wait for it to finish before starting a new one.")
        run_scan_now = False
    ...
    if run_scan_now:
        ...
        def evaluate_stock(ticker):
            ...
            try:
                ...
                df = get_cached_history(key, access_token, days=1200, fetch_fn=_fetch_upstox_history_impl)
        ...
        # Keep the script responsive even when several historical-data calls
        # are slow. The old as_completed(..., timeout=N) call blocked without
        # any UI update between completions; for 94 candidates its deadline
        # was 188 seconds, so the last completed ticker (for example BEL) made
        # the page look frozen for minutes. Polling wait() gives Streamlit a
        # heartbeat every second and the tighter wall-clock cap guarantees a
        # partial, honestly-labelled result instead of an apparent hang.
        OVERALL_SCAN_TIMEOUT_SECS = min(90, max(45, len(stage1_shortlist)))
        executor = concurrent.futures.ThreadPoolExecutor(max_workers=scan_workers)
        futures = {executor.submit(evaluate_stock, ticker): ticker for ticker in stage1_shortlist}
        completed = 0
        worker_exceptions = 0  # DIAGNOSTIC: separate from technical rejections AND timeouts, per requirement
        LOGGER.info("FULL_NSE_TRACE: submitted %d futures to ThreadPoolExecutor (scan_workers=%d)", len(futures), scan_workers)
        scan_started_at = time.monotonic()
        scan_deadline = scan_started_at + OVERALL_SCAN_TIMEOUT_SECS
        pending_futures = set(futures)
        try:
            while pending_futures:
                remaining_secs = scan_deadline - time.monotonic()
                if remaining_secs <= 0:
                    break

                done, _ = concurrent.futures.wait(
                    pending_futures,
                    timeout=min(1.0, remaining_secs),
                    return_when=concurrent.futures.FIRST_COMPLETED,
                )
                if not done:
                    elapsed_secs = int(time.monotonic() - scan_started_at)
                    progress_bar.progress(
                        completed / max(len(futures), 1),
                        text=(
                            f"Analyzed {completed}/{len(futures)} — waiting for "
                            f"{len(pending_futures)} data response(s) "
                            f"({elapsed_secs}s/{OVERALL_SCAN_TIMEOUT_SECS}s)"
                        ),
                    )
                    continue
                ...
                for future in done:
                    ...
                    progress_bar.progress(
                        completed / max(len(futures), 1),
                        text=f"Analyzed {completed}/{len(futures)} — latest: {ticker}",
                    )
                    try:
            ...
            # Anything still unprocessed at the wall-clock deadline is a real
            # timeout, including work that was queued behind a blocked worker.
            for future in pending_futures:
                ticker = futures[future]
                timed_out_tickers.append(ticker)
                rejection_counts["Data"] = rejection_counts.get("Data", 0) + 1
                if len(rejection_examples) < 40:
                    rejection_examples.append({"Ticker": ticker, "Category": "Data", "Reason": "Historical data request timed out"})
            if timed_out_tickers:
                LOGGER.warning(
                    "SLOW_SCAN: overall %ds deadline hit with %d/%d candidates still outstanding — "
                    "recorded as timeouts, not waited on further.",
                    OVERALL_SCAN_TIMEOUT_SECS, len(timed_out_tickers), len(futures)
                )
        finally:
            executor.shutdown(wait=False, cancel_futures=True)  # cancel_futures: Python 3.9+, confirmed safe (this deployment runs 3.12) — only cancels QUEUED-not-yet-started work, cannot stop an already-running blocked thread
            progress_bar.empty()
            # Release the process-wide scan lease even if Streamlit interrupts
            # this run, a worker raises, or result rendering fails afterward.
            # Previously release happened only at the very end of the success
            # path, leaving a stale yellow "another scan" warning for 150s.
            _scan_coordinator.finish(_scan_owner)
            if timed_out_tickers:
                st.warning(
                    f"⚠️ Scan completed with {len(timed_out_tickers)} timeout(s) after {OVERALL_SCAN_TIMEOUT_SECS}s — "
                    f"Upstox was slow to respond for: {', '.join(timed_out_tickers[:10])}"
                    f"{'...' if len(timed_out_tickers) > 10 else ''}. Other results below are unaffected."
                )
        LOGGER.info(
            "FULL_NSE_TRACE: completed=%d, worker_exceptions=%d, timed_out=%d, valid_signals=%d",
            completed, worker_exceptions, len(timed_out_tickers), len(valid_signals)
        )
        scan_timing["expensive_analysis_secs"] = round(time.perf_counter() - _t2, 2)
        scan_timing["timed_out_count"] = len(timed_out_tickers)
        ...
        # FIX (forensic trace, diagnostics-desync bug): these 4 keys were
        # previously SHARED across Quick and Full NSE — unlike
        # last_quick_signals/last_full_scan_signals, which were already
        # correctly mode-specific. A Full NSE run's diagnostics could
        # therefore be silently overwritten by, or fail to overwrite, a
        # Quick Scan's stale data depending on rerun timing. Now suffixed by
        # the ACTIVE scan_mode so each mode's diagnostics are permanently
        # isolated from the other's, matching the pattern already proven
        # correct for the signals themselves.
        diag_suffix = "full" if scan_mode.startswith("Full") else "quick"
        st.session_state[f"last_rejection_counts_{diag_suffix}"] = rejection_counts
        st.session_state[f"last_rejection_examples_{diag_suffix}"] = rejection_examples
        funnel_stats["live_quote_data_count"] = len(live_quote_data)
        funnel_stats["worker_exceptions"] = worker_exceptions
        funnel_stats["timed_out_count"] = len(timed_out_tickers)
        funnel_stats["futures_submitted"] = len(futures)
        funnel_stats["completed"] = completed
        st.session_state[f"last_funnel_stats_{diag_suffix}"] = funnel_stats
        st.session_state["last_diagnostics_scan_mode"] = scan_mode
        scan_timing["total_secs"] = round(sum(v for k, v in scan_timing.items() if k.endswith("_secs")), 2)
        st.session_state[f"last_scan_timing_{diag_suffix}"] = scan_timing

        if scan_mode.startswith("Full"):
            st.session_state["last_full_scan_signals"] = valid_signals
        else:
            st.session_state["last_quick_signals"] = valid_signals
        if funnel_stats.get("data_quality_failed"):
            scan_stage_status.error(
                f"Scan not accepted — quote coverage was {funnel_stats.get('quote_coverage_pct', 0):.1f}%, "
                "below the 90% Full-NSE data-quality requirement."
            )
        else:
            scan_stage_status.success(
                f"Scan complete — universe {funnel_stats.get('universe_size', 0):,}, "
                f"quotes {funnel_stats.get('quoted', 0):,}, shortlisted {funnel_stats.get('shortlisted', 0):,}, "
                f"deep analysis completed {funnel_stats.get('completed', 0):,}."
            )
    ...
    if rejection_counts or valid_signals or _scan_result_available:
        with st.expander(f"Diagnostics ▾ — {len(valid_signals)} passed, {total_rejected} rejected", expanded=not valid_signals):
            ...
            sm7.metric("Data Failures", rejection_counts.get("Data", 0) + rejection_counts.get("Error", 0))
            ...
            st.markdown("**Data issues (by category)** — count, % of analyzed candidates, and a sample")
            if rejection_counts:
                ...
                for i, (category, count) in enumerate(sorted(rejection_counts.items(), key=lambda x: -x[1])):
                    ...
                    rc_cols[i].metric(f"{label}", count, f"{pct:.0f}% of analyzed")
    ...
    if not _scan_result_available:
        ...
        _existing_timing = dict(st.session_state.get(f"last_scan_timing_{_diag_suffix}", {}))
        _existing_timing["ranking_secs"] = _ranking_secs
        _existing_timing["total_secs"] = round(_existing_timing.get("total_secs", 0.0) + _ranking_secs, 2)
        st.session_state[f"last_scan_timing_{_diag_suffix}"] = _existing_timing
...
# ==========================================
    ...
    all_mcx_tickers = sorted(mcx_dict)
    if not all_mcx_tickers or mcx_fut_df.empty:
        ...
        if mcx_key:
            ...
            if not hist_df.empty:
                ...
                if mcx_ltp > 0 and curr_atr > 0 and (bullish_setup or bearish_setup):
                    ...
                    st.success(
                        f"**Validated setup for {selected_commodity}** → {direction} @ ~₹{mcx_ltp:,.2f} "
        ...
        if st.button("Find Top Funds", key="mf_find_top_btn"):
            ...
            if not all_schemes:
                ...
                if not candidates:
                    ...
                    with st.spinner(
                        f"Loading five-year histories for {len(candidates)} funds and official AMFI costs in parallel..."
                    ):
                        ...
                        try:
                            ...
                            completed_data, pending_data = concurrent.futures.wait(
                                [history_future, ter_future], timeout=75
                            )
                    ...
                    if not results:
                        ...
                        st.success(
                            ...
                            f"{top['why_ranked']}. This is a historical category ranking, not a guaranteed recommendation."
                        )
                        top_cols = st.columns(5)
                        top_cols[0].metric("Composite Score", f"{top['score']:.0f}/100")
                        top_cols[1].metric("3Y CAGR", f"{top['cagr_3y']:+.2f}%" if pd.notna(top.get("cagr_3y")) else "N/A")
                        top_cols[2].metric("5Y CAGR", f"{top['cagr_5y']:+.2f}%" if pd.notna(top.get("cagr_5y")) else "N/A")
                        top_cols[3].metric("Maximum Drawdown", f"{top['max_drawdown']:.2f}%" if pd.notna(top.get("max_drawdown")) else "N/A")
                        top_cols[4].metric("Direct TER", f"{top['ter']:.2f}%" if pd.notna(top.get("ter")) else "N/A")

                        if all(pd.notna(top.get(field)) for field in ("forecast_p10", "forecast_p50", "forecast_p90")):
                            st.info(
                                f"Central 80% historical 12-month scenario range: **{top['forecast_p10']:+.1f}% to {top['forecast_p90']:+.1f}%** "
                                f"(median {top['forecast_p50']:+.1f}%; positive in {top['probability_positive']:.0f}% of resampled scenarios). "
                                f"Data confidence: **{top['confidence_label']}**. This is uncertainty analysis, not a promised forecast."
                            )
                        ...
                        for r in ranked[:10]:
                            rows.append({
                                ...
                                "5Y CAGR": f"{r['cagr_5y']:+.2f}%" if pd.notna(r.get("cagr_5y")) else "N/A",
                                "Rolling 3Y Median": f"{r['rolling_3y_median']:+.2f}%" if pd.notna(r.get("rolling_3y_median")) else "N/A",
                                "Max Drawdown": f"{r['max_drawdown']:.2f}%" if pd.notna(r.get("max_drawdown")) else "N/A",
                                "Sortino": round(float(r["sortino"]), 2) if pd.notna(r.get("sortino")) else "N/A",
                                "Peer IR*": round(float(r["peer_information_ratio"]), 2) if pd.notna(r.get("peer_information_ratio")) else "N/A",
                                "Direct TER": f"{r['ter']:.2f}%" if pd.notna(r.get("ter")) else "N/A",
                                "AUM": f"₹{float(r['aum_crore']):,.0f} Cr" if pd.notna(r.get("aum_crore")) else "N/A",
                                "Data Confidence": r["confidence_label"],
                                "Why Ranked": r["why_ranked"],
                            })
                        st.markdown(f"#### Top {len(rows)} {selected_mf_category} funds")
                        st.dataframe(pd.DataFrame(rows), width='stretch', hide_index=True)
                        st.session_state["last_mf_top_picks"] = {"category": selected_mf_category, "top": rows[:3]}
                        ter_coverage = sum(pd.notna(r.get("ter")) for r in ranked)
                        st.caption(
                            f"Evaluated {len(results)}/{len(candidates)} eligible funds; official TER matched for "
                            f"{ter_coverage}/{len(ranked)}. Completed in {time.perf_counter() - mf_scan_started:.1f}s. "
                            f"NAV as of {top['latest_date']}; TER as of {top.get('ter_date') or 'unavailable'}. "
                            f"*Peer IR compares monthly returns with the category median; "
                            f"it is not the official SEBI Tier-1 benchmark Information Ratio."
                        )
                        with st.expander("How the score and uncertainty range are calculated"):
                            scoring_weights = mf_scoring_weights(selected_mf_category)
                            st.markdown(
                                f"**Score:** {scoring_weights['returns']:.0%} returns, "
                                f"{scoring_weights['risk_adjusted']:.0%} risk-adjusted performance, "
                                f"{scoring_weights['downside']:.0%} downside protection, "
                                f"{scoring_weights['consistency']:.0%} consistency, "
                                f"{scoring_weights['cost']:.0%} cost efficiency and "
                                f"{scoring_weights['operational']:.0%} data depth/disclosed AUM. Every component is "
                                "converted to a percentile among funds in the selected category.\n\n"
                                "**Scenario range:** the 10th–90th percentile of 2,000 one-year paths created by resampling three-month blocks "
                                "from up to five years of monthly NAV returns. Estimates are shrunk toward the category "
                                "median when data confidence is lower. Data confidence measures history length, completeness "
                                "and freshness—not proven prediction accuracy.\n\n"
                                "Official Riskometer and Tier-1 TRI-relative IR are not shown until an authoritative, "
                                "scheme-matched feed is available; the application does not manufacture those values."
                            )
                ...
                if chosen_name:
                    ...
                    if stats:
                        ...
                        if all(stats.get(field) is not None for field in ("forecast_p10", "forecast_p50", "forecast_p90")):
                            st.caption(
                                f"Historical 12-month scenario: {stats['forecast_p10']:+.1f}% to {stats['forecast_p90']:+.1f}% "
            ...
            with st.spinner(f"Analyzing SMC & Technical Structure for {search_ticker}..."):
                ...
                if not s_df.empty:
                    ...
                    if atr_series.empty:
                        ...
                        st.markdown(f"### Institutional Setup for {search_ticker} — {bias_label}")
                        if smc_bias == "Neutral":
        ...
        with st.chat_message("assistant"):
            if not GEMINI_AVAILABLE:
                ...
                try:
                    ...
                    LOGGER.exception("Gemini request failed [%s]", error_id)
