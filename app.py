        ...
        for (order, label) in sorted(buckets.keys()):
            ...
            with st.expander(f"{label} ({len(bucket_signals)})", expanded=(order <= 1)):
                for sig in bucket_signals:
                    st.markdown(f"#### {sig['Ticker']} — {sig['Sector']}")
                    _render_risk_card(sig)
                    with st.expander("📊 Institutional Score Breakdown"):
                        _render_score_breakdown(sig)
                    with st.expander("🔍 Why Selected"):
                        _render_why_selected(sig)
                    st.markdown("---")
