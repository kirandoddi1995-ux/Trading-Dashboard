...
def get_full_nse_instrument_dictionary():
    try:
        ...
        # === DIAGNOSTIC INSTRUMENTATION (measurement only, zero logic change) ===
        raw_rows = len(df)
        ...
        # Split into two sequential steps (was one combined line) — purely to
        # measure exchange vs instrument_type independently. AND is order-
        # independent, so the FINAL filtered result is mathematically
        # identical to the original combined single-line version — this is
        # NOT a logic change.
        nse_df = df[df['exchange'] == 'NSE_EQ']
        LOGGER.info("NSE_UNIVERSE_DIAG: rows after exchange=='NSE_EQ' filter = %d (removed %d)", len(nse_df), raw_rows - len(nse_df))

        nse_df = nse_df[nse_df['instrument_type'] == 'EQUITY']
        LOGGER.info("NSE_UNIVERSE_DIAG: rows after instrument_type=='EQUITY' filter = %d", len(nse_df))
