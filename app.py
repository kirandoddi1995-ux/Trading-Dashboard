    ...
    if nifty_pct is not None:
        for ticker, key in keys.items():
            ...
            try:
                ...
                stock_rs[ticker] = momentum_pct - nifty_pct
            except Exception as e:
                LOGGER.debug("Suppressed exception: %s", e)
                continue
...
# ==========================================
    ...
    with st.expander("🏭 Sector Strength Score", expanded=True):
        st.caption("Which sectors are genuinely leading or lagging the market right now, ranked by average Relative Strength vs NIFTY (not just raw price moves) across liquid stocks in each sector.")
        try:
        ...
        if sector_rows:
            ...
            st.caption("Sector strength unavailable right now (no quote or historical data reachable).")
        ...
        if st.button("Find Top Funds", key="mf_find_top_btn"):
            ...
            if not all_schemes:
                ...
                if not candidates:
                    ...
                    # Was doing 60 sequential HTTP round-trips (one fund at a time) —
                    # this could take 30-120+ seconds, which read as "not giving any
                    # suggestions" even though it was technically still working, just
                    # very slow. Parallelized to match the pattern used everywhere
                    # else in this app for bulk fetches.
                    with concurrent.futures.ThreadPoolExecutor(max_workers=15) as executor:
