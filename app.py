...
if selected_tab == "Options & Derivatives Chain":
    ...
    if not using_live_chain:
        ...
        for rank, r in enumerate(recommendations, start=1):
            ...
            dte_display = r.get("dte")
            dte_tier_display = r.get("dte_tier", "")
            dte_warning = ""
            if dte_display is not None:
                if dte_display <= 1:
                    dte_warning = " ⚠️ Expiry tomorrow" if dte_display == 1 else " ⚠️ Expiry today"
                elif dte_display <= 2:
                    dte_warning = " ⚠️ Expiry approaching"
            st.success(
                f"**#{rank} · {label} · {r['bias']} bias · DTE {dte_display} ({dte_tier_display}){dte_warning}** → BUY **{selected_opt_asset} {int(r['strike'])} {r['side']}** "
        ...
        if recommendations:
            st.caption(
                "Ranked by reward:risk across ATM and nearby OTM strikes on the biased side. Target/stop bands are now "
                "DTE-aware — tighter as expiry approaches, since less time remains for the theoretical move to play out. "
        ...
        if selected_detail_ticker != "— Select —":
            ...
            if detail_sig:
                ...
                with st.expander("📈 Technical Details (EMA, RSI, ADX, ATR, SuperTrend, MACD, RVOL, etc.)", expanded=False):
                    ...
                    st.dataframe(pd.DataFrame([detail_sig])[detail_technical_cols], use_container_width=True, hide_index=True)
