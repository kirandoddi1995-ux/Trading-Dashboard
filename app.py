...
# ==========================================
    ...
    st.caption("What kind of market this is right now (trending, choppy, panicky) and how broad-based the move is among large/mid-caps — the context to weigh before trusting any signal below.")
    try:
    ...
    # Rejection Statistics — always visible, satisfies "always understand WHY"
    # regardless of whether the scan found anything. Built from the exact same
    # thresholds evaluate_stock already applies — this only reports the
    # existing decisions, it doesn't change them.
    total_rejected = sum(rejection_counts.values())
    if rejection_counts or valid_signals:
        with st.expander(f"📊 Screening Transparency — {len(valid_signals)} passed, {total_rejected} rejected", expanded=not valid_signals):
            if rejection_counts:
                ...
                for i, (category, count) in enumerate(sorted(rejection_counts.items(), key=lambda x: -x[1])):
                    ...
                    rc_cols[i].metric(f"{label} Failures", count)
            ...
            if rejection_examples:
                ...
                st.dataframe(pd.DataFrame(rejection_examples), use_container_width=True, hide_index=True)
    ...
    if valid_signals:
        ...
        hidden_cols = ['score', '_price_val', '_qty', '_risk_amt', '_returns', '_sector', 'Risk %', 'Exp Return', 'Risk:Reward']
        df_display_cols = [c for c in pd.DataFrame(valid_signals).columns if c not in hidden_cols]

        # Requirement 2/4: Institutional Score buckets instead of a flat
        # pass/fail list. Uses the EXISTING "Signal Strength" field (already
        # computed, unchanged) — this only changes how results are GROUPED for
        # display, not which stocks passed or what their score is.
        def _bucket_for(sig_strength):
            if sig_strength >= 85:
                return "🏆 Elite Setups", 0
            elif sig_strength >= 70:
                return "🟢 High Conviction", 1
            elif sig_strength >= 55:
                return "🟡 Watchlist", 2
            else:
                return "⚪ Developing Setups", 3

        buckets = {}
        for sig in valid_signals:
            label, order = _bucket_for(sig.get("Signal Strength", 0))
            buckets.setdefault((order, label), []).append(sig)

        st.markdown(f"##### {len(valid_signals)} Institutional Equities — {custom_days}-Day Horizon (Under ₹{max_stock_price:,.0f})")
        bucket_thresholds_caption = "Bucketed by Signal Strength: Elite 85+, High Conviction 70-84, Watchlist 55-69, Developing below 55 — same underlying score as before, just grouped for faster scanning."
        st.caption(bucket_thresholds_caption)
        for (order, label) in sorted(buckets.keys()):
            bucket_signals = buckets[(order, label)]
            with st.expander(f"{label} ({len(bucket_signals)})", expanded=(order <= 1)):
                bucket_df = pd.DataFrame(bucket_signals)[df_display_cols]
