...
# ==========================================
    ...
    if run_scan_now:
        ...
        # HANG FIX: previously `with ThreadPoolExecutor(...) as executor:` —
        # the context manager's __exit__ calls shutdown(wait=True)
        # UNCONDITIONALLY, blocking the Streamlit script until EVERY submitted
        # future finishes, no matter how they're iterated inside. A single
        # slow/hung historical-data request (worst case ~33s per attempt-set:
        # 3 retries x 10s timeout + backoff — see investigation) occupying a
        # worker thread could not be bounded by anything done INSIDE that
        # block, since future.result(timeout=N) only raises locally — it does
        # not, and cannot, kill the underlying blocked thread. Fixed by:
        # (1) NOT using the context manager, so exiting this scope never waits
        # on stragglers, (2) an OVERALL deadline on as_completed() itself,
        # bounding total wait regardless of how many candidates are still
        # outstanding, (3) explicit shutdown(wait=False) — releases the
        # executor object without blocking on any still-running thread.
        # Straggler threads (rare — only when Upstox itself is critically
        # slow) continue in the background and simply have their results
        # discarded when they eventually finish; they cannot be forcibly
        # killed in Python's threading model without a much larger
        # process-based rewrite, which is out of scope for this fix.
        OVERALL_SCAN_TIMEOUT_SECS = min(240, max(60, len(stage1_shortlist) * 2))
        ...
        try:
            for future in concurrent.futures.as_completed(futures, timeout=OVERALL_SCAN_TIMEOUT_SECS):
                completed += 1
            ...
            # Overall deadline hit — record every still-outstanding candidate
            # as a genuine timeout (Requirement D: never a fake success), not
            # silently dropped, and not treated as a normal rejection category.
            for f, ticker in futures.items():
                if not f.done():
