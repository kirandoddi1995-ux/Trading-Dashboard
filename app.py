...
try:
    EXCHANGE_STATUS = fetch_exchange_market_status(ACTIVE_EXCHANGE, access_token)
except Exception as exc:
    EXCHANGE_STATUS = {}
...
def get_live_market_quotes(keys_list, token):
    ...
    if buffer is not None and MARKET_OPEN:
        ...
        snap={key: quote for key, quote in buffer.snapshot(keys_list).items()
...
# ==========================================
    ...
    recommendations = (generate_ranked_recommendations(market_bias)
                       if using_live_chain and MARKET_OPEN and not using_stale_price else [])
        ...
        def evaluate_stock(ticker):
            ...
            try:
                ...
                if sl is None or tgt is None or not 0 < sl < price < tgt:
                    return None, {"category": "Risk:Reward", "reason": "Invalid structural trade levels"}
                ...
                if rr_ratio < 1.35:
                    return None, {"category": "Risk:Reward", "reason": f"Net reward:risk {rr_ratio:.2f} is below 1.35 after estimated 0.30% costs"}
                if risk_per_share <= 0:
                ...
                return None, {"category": "Error", "reason": f"Analysis error: {type(exc).__name__}"}
