...
@st.cache_data(ttl=300, show_spinner=False)
def compute_market_regime(nifty_df, nifty_quote, vix_val, breadth_score):
    """Rule-based (not ML) market regime classifier using NIFTY's own trend (ADX,
    EMA slope), India VIX, and market breadth — all data already computed
    elsewhere in the app, just combined here into one read. Honest about what
    this is: a transparent, inspectable set of thresholds, not a trained model.
    ...
    False-confidence fix: also returns a continuous regime_score (0-100) — how
    far ADX sits from the classification threshold — so a hard label flip at an
    arbitrary cutoff (e.g. ADX 24.9 vs 25.0) isn't presented as equally certain
    as a clearly trending market. This is a heuristic distance measure, not a
    statistically calibrated confidence — labeled as such in the UI."""
    try:
        ...
        adx_val = float(adx_df.iloc[-1, 0]) if adx_df is not None and not adx_df.empty and pd.notna(adx_df.iloc[-1, 0]) else None
        ...
        # Classification thresholds — transparent and adjustable, not hidden in a model.
        ...
        elif vix_val is not None and vix_val <= 11 and breadth_score is not None and breadth_score >= 70:
            regime = "EUPHORIA"
        ...
        # Heuristic continuous score: distance of ADX from the 25 trending
        # threshold, centered at 50 so values near the cliff-edge read as
        # low-confidence rather than a false-precise flip.
        regime_score = 50.0
        if adx_val is not None:
            regime_score = min(100.0, max(0.0, 50.0 + (adx_val - 25.0) * 4.0))
...
def get_full_nse_instrument_dictionary():
    try:
        ...
        # with a digit. This was previously gated behind a length<=8 heuristic that
        # missed real-world cases like "1003IIFL29" (10 chars) — digit-prefix alone
        # is a reliable signal on its own, so apply it directly and unconditionally.
...
# ==========================================
    ...
    if scan_mode.startswith("Full"):
        ...
        if not already_scanned_this_session:
            ...
            run_scan_now = st.button("🔄 Refresh Results", key="eq_run_quick_btn", use_container_width=True)
            if not run_scan_now:
    ...
    with st.expander("🌐 Market Regime & Breadth (market context — optional)", expanded=False):
        st.caption("What kind of market this is right now (trending, choppy, panicky) and how broad-based the move is among large/mid-caps. Informational only — doesn't affect the stock picks above.")
        try:
        ...
        try:
            ...
            regime = compute_market_regime(nifty_hist_df, nifty_quote, vix_value, breadth["breadth_score"] if breadth else None)
        except Exception as exc:
        ...
        if regime:
            ...
            rc2.metric("Regime Score", f"{regime['regime_score']:.0f}/100")
            rc3.metric("NIFTY ADX (14)", f"{regime['adx']:.1f}" if regime['adx'] is not None else "N/A")
            rc4.metric("India VIX", f"{regime['vix']:.1f}" if regime['vix'] is not None else "N/A")
            st.caption("Regime Score is a heuristic distance from the trending threshold (50 = right at the boundary), not a calibrated statistical confidence.")
    ...
    with st.expander("🏭 Sector Strength Score (market context — optional)", expanded=False):
