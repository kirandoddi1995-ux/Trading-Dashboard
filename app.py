...
APP_BUILD = "v14.6-MUTUAL-FUND-RECOVERY"
...
def _parse_tigzig_scheme_snapshot(snapshot_bytes):
    ...
    columns = ["scheme_code", "scheme_name", "amc", "is_active"]
    frame = pd.read_csv(
        io.BytesIO(payload),
    ...
    for row in frame.itertuples(index=False):
        schemes.append({
            "schemeCode": str(getattr(row, "scheme_code")),
            "schemeName": str(getattr(row, "scheme_name")),
            "fundHouse": str(getattr(row, "amc", "N/A") or "N/A"),
...
def _fetch_mf_scheme_list_cached():
    ...
    # Authoritative first choice: AMFI's current daily NAV file.
    try:
        with get_robust_session(total_retries=1) as session:
    ...
    # Independent AMFI mirror; one small compressed request instead of pagination.
    try:
        with get_robust_session(total_retries=1) as session:
...
def _fetch_mf_nav_history_cached(scheme_code):
    """Fetch three-plus years of NAV history; transient failures are not cached."""
    code = str(scheme_code).strip()
    since = (datetime.date.today() - datetime.timedelta(days=(366 * 3) + 14)).isoformat()
    provider_errors = []
...
# ==========================================
    ...
    st.markdown("AMFI NAV data with automatic provider fallback, so one service outage does not disable this page.")
                    ...
                    def _fetch_and_score(c):
                        ...
                        if stats:
                            ...
                            if stats.get("scheme_name") in {None, "", "N/A"}:
                                stats["scheme_name"] = c.get("schemeName", "N/A")
                            if stats.get("fund_house") in {None, "", "N/A"}:
                                stats["fund_house"] = c.get("fundHouse", "N/A")
                            return stats
                        return None
