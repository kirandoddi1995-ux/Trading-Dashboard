...
# ==========================================
    ...
    if NSE_UNIVERSE_VALIDATION_FAILED:
        st.error(
            f"⚠️ **NSE instrument universe validation failed** — only {len(all_nse_tickers)} instruments loaded, "
            f"expected several thousand for a genuinely complete NSE equity universe. This indicates a loading/"
            f"filtering bug, not a legitimately small NSE. **Full NSE scans will be materially incomplete** until "
            f"this is resolved — Quick Scan (which uses its own separate liquid-ticker list) is unaffected. "
            f"Check Streamlit Cloud's logs for lines starting with `NSE_UNIVERSE_DIAG:` for the exact cause."
        )
