...
APP_BUILD = "v14.0-PRODUCTION-HARDENED"
...
ALLOW_INSECURE_LOCAL_DEV = os.environ.get("ALLOW_INSECURE_LOCAL_DEV", "0").strip().lower() in {"1", "true", "yes"}

if not ALLOW_INSECURE_LOCAL_DEV:
    if not hasattr(st, "login") or not hasattr(st, "user"):
        st.error("Authentication requires Streamlit 1.42 or newer. Upgrade Streamlit before running this application.")
        st.stop()
    if not _oidc_is_configured():
        st.error(
            "Authentication is not configured. Add an OIDC provider under `[auth]` in Streamlit Secrets. "
            "The application is intentionally fail-closed until authentication is configured."
        )
        st.caption("Use `.streamlit/secrets.example.toml` as the configuration template. Never commit the real secrets file.")
        st.stop()
    if not st.user.is_logged_in:
        st.title("Secure Quant Terminal")
        st.write("Sign in to access market analysis and your isolated portfolio data.")
        if st.button("Sign in", type="primary"):
            st.login()
        st.stop()

    _identity_claim = st.user.get("sub") or st.user.get("email")
    if not _identity_claim:
        st.error("The identity provider did not return a stable subject or email claim. Access has been denied.")
        st.stop()
    CURRENT_USER_ID = hashlib.sha256(str(_identity_claim).encode("utf-8")).hexdigest()
    CURRENT_USER_DISPLAY = str(st.user.get("name") or st.user.get("email") or "Authenticated user")
else:
    CURRENT_USER_ID = hashlib.sha256(b"local-development-only").hexdigest()
    CURRENT_USER_DISPLAY = "Local development"
    st.warning("Local development authentication bypass is active. Never set ALLOW_INSECURE_LOCAL_DEV in production.")
...
st.sidebar.caption(f"Signed in: {CURRENT_USER_DISPLAY}")
if not ALLOW_INSECURE_LOCAL_DEV and st.sidebar.button("Sign out", key="auth_logout"):
    st.logout()
...
if access_token:
    st.sidebar.success("UPSTOX TOKEN: CONFIGURED")
else:
...
class ScanCoordinator:
    ...
    block every user indefinitely.  It coordinates work only; scan results and
    user data remain isolated in each authenticated Streamlit session.
    """
...
if selected_tab == "Settings":
    ...
    st.caption("Account, risk, data-source and production-safety configuration.")
    sec1, sec2, sec3 = st.columns(3)
    sec1.metric("Authentication", "Enabled" if not ALLOW_INSECURE_LOCAL_DEV else "Local bypass")
    ...
    st.markdown(
        ...
        "- Use OIDC authentication with a strong cookie secret and least-privilege provider client.\n"
        "- Store persistent personal data in a managed database with encrypted backups and per-user authorization.\n"
    ...
    if run_scan_now:
        ...
        def evaluate_stock(ticker):
            ...
            try:
                ...
                probability_result = compute_historical_setup_probability(df_clean, horizon_days=custom_days)
                analysis_timing_log.append(("historical_probability", time.perf_counter() - _prob_t0))
                historical_win_prob = probability_result['win_probability'] if probability_result else None
                probability_ci = (
                    f"{probability_result['ci_low']:.1f}–{probability_result['ci_high']:.1f}%"
                    if probability_result else "N/A"
                )
                ...
                edge_score = _scale(probability_result['ci_low'], 35.0, 65.0) if probability_result else 50.0
