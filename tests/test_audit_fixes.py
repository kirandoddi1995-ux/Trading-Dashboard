...
def app_functions(*names, **context):
    ...
    namespace = dict(pd=pd, np=np, math=math, time=time, datetime=datetime,
                     threading=threading, runtime=runtime, trade_contracts=trade_contracts,
                     option_rr_rejections=[], market_bias_scores={"net_score": -75},
                     ta=ta, LOGGER=logging.getLogger('tests'))
...
def test_option_costs_depth_and_capital_use_one_execution_model():
    fixed_now = datetime.datetime(2026, 9, 2, 10, 0, tzinfo=datetime.timezone(datetime.timedelta(hours=5, minutes=30)))
    ...
    assert result['stop_was_tightened'] is True
    row['_put_ask_qty']=65
